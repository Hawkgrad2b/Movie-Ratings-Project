...
def demRatingPrediction(u, m, userList, rLu):    #this one is just a little bit off for u = 1 and u = 2 for j =123 and 1
...
def genreRatingPrediction(u, m, movieList, rLu):
    ...
    # refGenre = movieList[m-1]["genre"]
    # refGenreIndexes = [i for i, num in enumerate(refGenre) if num == 1]

    # subMovies = []
    # for movieIndex, movie in enumerate(movieList):
    #     if movieIndex == m-1:
    #         continue
    #     movieGenreIndexes = [i for i, num in enumerate(movie["genre"]) if num == 1]
        
        
    #     if any(genreIndex in movieGenreIndexes for genreIndex in refGenreIndexes):
    #         subMovies.append(movieIndex)
            
    # if set(refGenreIndexes).intersection(set(movieGenreIndexes)):
            #subMovies.append(movieIndex)     
    #return the mean of all the movies that user "u" has rated that are in subMovies         and could be more effiencient
    # for movie in subMovies:
    #     if movie != m-1:
    #         for i in rLu[u-1].items():
    #             if i[0] == (movie+1):
    #                 totalRatings += i[1]
    #                 numRatings += 1
    
####   this is faster way, but still a little wrong
    # ratingsU = rLu[u-1]
    # for m in ratingsU.items():
    #     if (m[0] != m) and m[0] in subMovies:
    #         totalRatings += m[1]
    #         numRatings += 1
####
...
#check to see if actual Ratings is a list of ratings or tuples
...
#     #actualRatings = createActualRatings(rawRatings)
...
#         #split into training and test using partition function
#         [trainingSet, testingSet] = partitionRatings(rawRatings, 20)
        
#         #call createRatingsData on the training set to create TrRLu and TrRLm
#         [TrRLu, TrRLm] = createRatingsDataStructure(numUsers, numItems, trainingSet)
        
#         #create list to predictedRatings for each of the 5 functions, using (u, m, r) in each function
...
#         #call RMSE on all 5 lists        
#         #if testing set needs be just a list and not tuples, call actualRatings() and replace testing set with the current trainingSet list of ratings
#         #testingSet is replaced by actualRatings ?
...
#     return[randomPredictionListRMSE, meanUserRatingPredictionListRMSE, meanMovieRatingPredictionListRMSE, demRatingPredictionListRMSE, genreRatingPredictionListRMSE]
#     #put graph code right above this and the copy everything from start down to end of graph code
#     #repeat same process 10 times (just copy and paste)
